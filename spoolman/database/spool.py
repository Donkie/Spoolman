...
async def use_weight(db: AsyncSession, spool_id: int, weight: float) -> models.Spool:
    ...
    if spool.first_used is None:
        spool.first_used = datetime.now(timezone.utc).replace(tzinfo=None, microsecond=0)
    spool.last_used = datetime.now(timezone.utc).replace(tzinfo=None, microsecond=0)
