...
class CalibrationStepResult(BaseModel):
    ...
    def from_db(item: models.CalibrationStepResult) -> "CalibrationStepResult":
        ...
        return CalibrationStepResult(
            ...
            inputs=json.loads(item.inputs) if item.inputs is not None else None,
            outputs=json.loads(item.outputs) if item.outputs is not None else None,
            selected_values=json.loads(item.selected_values) if item.selected_values is not None else None,
            notes=item.notes,
