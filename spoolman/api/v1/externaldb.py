...
async def profile(profile_id: str):
    ...
    try:
        ...
        logger.error("Failed to fetch filament profile %s: %s", profile_id, e)
        raise HTTPException(status_code=400, detail="Failed to fetch filament profile")
    ...
    for line in html.splitlines():
        if '\\"brand_name\\":\\"' in line:
            ...
            if material_match: data['material'] = material_match.group(1)

            material_type_match = re.search(r'\\"material_type\\":\\"([^"\\]+)\\"', line)
            if material_type_match: data['material_type'] = material_type_match.group(1)

            color_match = re.search(r'\\"color\\":\\"([^"\\]+)\\"', line)
            if color_match: data['color'] = color_match.group(1)

            rgb_match = re.search(r'\\"rgb\\":\\"#([^"\\]+)\\"', line)
            if rgb_match: data['color_hex'] = rgb_match.group(1)

            density_match = re.search(r'\\"density\\":([\d\.]+)', line)
            if density_match: data['density'] = float(density_match.group(1))

            diameter_match = re.search(r'\\"diameter\\":([\d\.]+)', line)
            if diameter_match: data['diameter'] = float(diameter_match.group(1)) / 1000.0

            weight_match = re.search(r'\\"nominal_weight\\":([\d\.]+)', line)
            if weight_match: data['weight'] = float(weight_match.group(1))

            spool_weight_match = re.search(r'\\"spool_weight\\":([\d\.]+)', line)
            if spool_weight_match: data['spool_weight'] = float(spool_weight_match.group(1))
            ...
            name_parts = []
            if 'color' in data: name_parts.append(data['color'])
            if 'material_type' in data and data['material_type']: name_parts.append(data['material_type'])
            if 'material' in data: name_parts.append(data['material'])
            if name_parts:
            ...
            if data_color_match:
                ...
                if len(color_hexes_raw) > 1:
                    ...
                    # By default use longitudinal for gradual or coaxial, since we can't tell, but coaxial is a good fallback for multi-color.
