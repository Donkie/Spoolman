...
Duplicated from tests_integration/tests/_openprinttag_fixtures.py rather than imported:
this package is mounted into its own container as a standalone `pytest tests_tagcreate`
target (see docker-compose-sqlite-tagcreate.yml), not as a subpackage of `tests/`, so it
has no relative import path back to it.
"""
