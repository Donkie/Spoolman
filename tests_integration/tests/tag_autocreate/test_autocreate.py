"""Integration tests for POST /tag/scan's opt-in create-and-link path, server-side enabled.

Only runs against docker-compose-sqlite-tagcreate.yml. Every other integration compose file
leaves SPOOLMAN_TAG_AUTO_CREATE_ENABLED unset, so `create: true` is inert there -- pinned in
tests_integration/tests/tag/test_scan_decode.py, in
test_create_flag_does_nothing_when_the_server_has_not_opted_in. This file is the other half:
with the server opted in, `create: true` on an unmatched, decodable tag actually creates a
spool and links the tag to it.
"""
...
def test_create_true_on_an_unmatched_decodable_tag_creates_and_links_a_spool():
    ...
    try:
        ...
        _delete_spool(body["matched_spool_id"])


def test_create_false_still_does_nothing_even_though_the_server_opted_in():
    """The server-side gate only ever removes a block; a caller still has to ask."""
    uid = _uid()
