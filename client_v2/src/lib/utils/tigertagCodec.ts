...
/**
 * Map a Spool (plus its Filament) to TigerTag binary data, mirroring the
 * Python map_spool_to_tigertag().
 */
export function mapSpoolToTigerTag(
	...
	// Color — the first swatch color for a multi-color filament.
	const colorHex = filament.colors[0];
	if (colorHex) {
